...
function generateOutput(
  ...
  // Read existing data.ts to preserve comparison data and test suite.
  // We regenerate: file header, types, EDU constants, and the occupation array.
  // Everything from "// ─── AT vs US Comparison Data" onward is preserved as-is.
  const existingDataTs = readFileSync(join(__dirname, "..", "src", "lib", "data.ts"), "utf-8");
  const comparisonMarker = "// ─── AT vs US Comparison Data";
  const comparisonIdx = existingDataTs.indexOf(comparisonMarker);
  const preservedSection = comparisonIdx !== -1 ? existingDataTs.slice(comparisonIdx) : "";
  ...
  const occEntries = occupations
    ...
    source: ${JSON.stringify(o.employmentSource + " | Pay: " + o.paySource + " | Pipeline: scripts/generate-occupations.ts → deterministic build, all source data publicly downloadable (OGD), 99 hypothesis-driven verification tests")},
  }`
    )
    .join(",\n");
